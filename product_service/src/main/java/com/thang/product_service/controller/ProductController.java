...
public class ProductController {
    ...
    @PostMapping("/search")
    public ResponseEntity<ApiResult<List<ProductDTO>>> search(
            @RequestBody ProductFilter productFilter) {
        return ResponseEntity.ok(
                ApiResult.success(productService.getProductsByIds(productFilter), "Products fetched successfully"));
    }
    ...
    public ResponseEntity<ApiResult<ProductDTO>> deductProduct(
            @PathVariable UUID id, @RequestBody @Valid ProductDeductRequest request) {
        log.info("PATCH {}/{}", ApiRoutes.PRODUCTS, id);
        return ResponseEntity.ok(
